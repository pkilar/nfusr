...
using statsCb = std::function<void(std::shared_ptr<nfusr::Logger>, const char*)>;

class ClientStats {
 public:
  ClientStats() : enabled_(false) {}
  ~ClientStats();
  int start(const char* fileName, const char* prefix, unsigned interval_seconds = 60);
  ...
  void recordOperation(enum fuse_optype optype, std::chrono::microseconds elapsed);
