...
void NfsCachedClient::writeCache(
  ...
  *cache_fname += ".swp";
  ...
  auto fid = ::open(data_fname->c_str(), O_RDWR | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
  if (fid < 0) {
    perror("open");
    ::close(fid);
    return;
...
void NfsCachedClient::replyEntry(
  ...
  if (local_cache_path) {
    ...
    if (pii->hasLocalCachePath()) {
      this->getLogger()->LOG_MSG(
          LOG_DEBUG, "%s Attempting to create directory %s.\n", __func__, pii->getLocalCachePath()->c_str());
      ...
      if (::stat(pii->getLocalCachePath()->c_str(), &s)) {
        ...
        if ((s.st_mode & S_IFDIR) && (s.st_mode & S_IRWXU)) {
          local_cache_path_ = std::make_shared<std::string>(*pii->getLocalCachePath());
  ...
  NfsClient::replyEntry(ctx, fh, attr, file, local_cache_path_, caller, parent, name);
}

void NfsCachedClient::read(fuse_req_t req, fuse_ino_t inode, size_t size, off_t off, struct fuse_file_info* file) {
  ...
  if (inodeObj->hasLocalCachePath()) {
    ...
    if (cache.readCache()) {
      this->getLogger()->LOG_MSG(
          LOG_DEBUG, "%s Reading inode %lu from cache file %s.\n", __func__, inode, path->c_str());
      fuse_reply_buf(req, cache.getData(), cache.getSize());
      return;
    }
  }
  this->getLogger()->LOG_MSG(LOG_DEBUG, "%s Reading inode %lu over network.\n", __func__, inode);
  auto ctx = new CachedReadRpcContext(this, req, FOPTYPE_READ, inode, file, size, off);
